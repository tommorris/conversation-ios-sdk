    ...
    override func spec() {
        ...
        context("register device") {
            it("should pass") {
                ...
                let success = try? self.client.appLifecycle.push.update(deviceToken: token, deviceId: "device")
                    .toBlocking()
                    .first()
            ...
            it("should fail when device id is empty") {
                ...
                guard let token = "token".data(using: .utf8) else { return fail() }
                let success = self.client.appLifecycle.push.update(deviceToken: token, deviceId: nil)
