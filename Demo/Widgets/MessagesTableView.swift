...
public class MessagesTableView: UITableView, UITableViewDataSource, UITableViewDelegate {
    ...
    private func setup() {
        ...
        messages.append(contentsOf: conversation.events.filter {
            $0 is TextEvent ||
            $0 is ImageEvent ||
            $0 is MediaEvent
        })
    ...
    private func bindUI() {
        /* Attach to events. */
        conversation?.events.newEventReceived.addHandler { [weak self] _ in
            DispatchQueue.main.async { self?.resetDataSource() }
        }
        
        conversation?.events.events.addHandler { [weak self] (_, type) in
            switch type {
            case .reset: break
    ...
    private func resetDataSource() {
        ...
        messages = conversation.events.filter { $0 is TextEvent || $0 is ImageEvent || $0 is MediaEvent }
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cell == nil {
            ...
            if let imageEvent = message as? ImageEvent {
                if let imageCell = cell as? MessageBubbleImageView {
                    imageCell.refreshImage(imageEvent: imageEvent)
                }
            }
        }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let event = messageForRowAtIndexPath(indexPath: indexPath) as? TextEvent {
            delegateConversationView?.onShowMessageDetail(message: event)
        }
    }
     ...
     */
    private func messageForRowAtIndexPath(indexPath: IndexPath) -> EventBase {
