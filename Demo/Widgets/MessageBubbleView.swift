...
public class MessageBubbleView: UITableViewCell {
    ...
    private static let MessageLabelFont: UIFont = UIFont.systemFont(ofSize: 12.0)
    ...
    /* Properties. */
    private(set) var message: EventBase?
    ...
    static func getMessageType(message: EventBase) -> MessageType {
        if let event = message as? ImageEvent {
            return .image(event)
        } else if let event = message as? TextEvent {
            return .text(event)
        }
     ...
     */
    static func Factory(onLeft: Bool, message: EventBase, parentWidth: CGFloat, avatarEnabled: Bool, name: String?) -> MessageBubbleView {
        switch getMessageType(message: message) {
     ...
     */
    public init(onLeft: Bool, message: EventBase, parentWidth: CGFloat, avatarEnabled: Bool, name: String?) {
    ...
    /**
     ...
     - returns: The width.
     */
    private static func CalculateContainerWidth(message: EventBase, parentWidth: CGFloat, avatarEnabled: Bool) -> CGFloat {
        ...
        if payloadWidth > 0 {
            ...
            let text = (message as? TextEvent)?.text ?? ""  
     ...
     */
    static func payloadWidthForMessage(message: EventBase, availableWidth: CGFloat) -> CGFloat {
    ...
    static func cellHeightForMessage(message: EventBase, parentWidth: CGFloat, avatarEnabled: Bool, showName: Bool) -> CGFloat {
    ...
    private static func LastCharPosition(text: String, availableWidth: CGFloat, font: UIFont) -> CGFloat {
        ...
        let attributedString = NSAttributedString(string: text, attributes: [NSAttributedStringKey.font: font])
        ...
        let location = layoutManager.location(forGlyphAt: text.count - 1)
        return location.x
    }

    public func updateMessage(onLeft: Bool, message: EventBase, name: String?) {
        self.onLeft = onLeft
        self.name = name
        updateUIwithMessage(message: message)
    }
    ...
    /**
     ...
     - parameter message: The message.
     */
    private func updateUIwithMessage(message: EventBase) {
        self.message = message
        
        /* Mark as seen (if it's a message sent to us). */
        if let event = message as? TextEvent {
            event.markAsSeen()
        }
        ...
        } else if let event = message as? MediaEvent {
            let state = event.enabled ? "enabled" : "disabled"
            
            messageText.text = "\(event.fromMember.user.displayName) \(state) audio"
        }
        ...
        if case .system = MessageBubbleView.getMessageType(message: message) {
            container.backgroundColor = UIColor(red: 1, green: 0.7098, blue: 0.7098, alpha: 1.0) /* #ffb5b5 */
        }
    ...
    private func messageStatusChanged(_ status: ReceiptRecord) {
        updateMessageStatus()
    }
