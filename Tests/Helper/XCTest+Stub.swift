...
internal extension XCTest {
    ...
    /// - returns: Stub
    @discardableResult
    internal func stub(file: AssetsTest, request: URLRequest?, statusCode: Int = 200) -> Stub {
        guard let fileURL = Bundle(for: type(of: self)).url(forResource: AssetsTest.nexmo.path, withExtension: nil),
        ...
        return stub(uri(url), Mockingjay.http(statusCode, download: Download.content(data)))
    }
    ...
    /// Convert JSON file in bundle to array object
    ///
    /// - parameter file: path of json
    ///
    /// - returns: dictionary
    internal func array(path: JSONTest) -> [Any] {
        guard let path = Bundle(for: type(of: self)).path(forResource: path.rawValue, ofType: "json") else { fatalError() }
        let fileUrl = URL(fileURLWithPath: path)

        guard let data = try? Data(contentsOf: fileUrl) else { fatalError() }
        guard let json = try? JSONSerialization.jsonObject(with: data, options: .allowFragments),
            let array = json as? [Any] else { fatalError() }
