...
internal class TextEventTest: QuickSpec {
    ...
    override func spec() {
        ...
        it("delete own event fails") {
            ...
            do {
                try client.storage.databaseManager.event.deleteAll()
                try client.storage.databaseManager.event.insert(textEvent.data)
        ...
        it("marks text a seen") {
            let mock = SimpleMockDatabase()
            let textEvent = TextEvent(conversationUuid: mock.conversation1.rest.uuid, event: mock.event1, seen: false)
            
            expect(textEvent.markAsSeen()) == true
            
            textEvent.refreshReceipts()
            
            expect(textEvent.allReceipts.isEmpty) == false
        }
    }
}
