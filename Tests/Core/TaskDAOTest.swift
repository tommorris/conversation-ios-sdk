...
internal class TaskDAOTest: QuickSpec {
    ...
    override func spec() {
        it("fetches number of pending task") {
            ...
            let relatedId = EventBase.uuid(from: event.id, in: event.cid)
            
            task.related = relatedId
            ...
            expect(self.databaseManager.task.taskExist(.send, for: relatedId)) == true
        ...
        it("removes all records") {
            ...
            expect { try self.databaseManager.task.deleteAll() }.toNot(throwAssertion())
            expect(self.databaseManager.task.taskExist(.send, for: relatedId)) == false
        }
