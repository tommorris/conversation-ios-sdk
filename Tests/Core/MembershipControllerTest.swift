...
internal class MembershipControllerTest: QuickSpec {
    ...
    override func spec() {
        ...
        it("invites a user to a conversation") {
            self.stub(file: .inviteUser, request: MembershipRouter.invite(username: "test", conversationId: "conversation").urlRequest)
            
            var completed = false

            _ = self.membershipController.invite(userId: "USR-123", username: "Test", for: "conversation", with: nil).subscribe(onNext: {
                completed = true
            })
        ...
        it("fails to invites a user to a conversation") {
            self.stubServerError(request: MembershipRouter.invite(username: "test", conversationId: "conversation").urlRequest)
            
            var networkError: Error?

            _ = self.membershipController.invite(userId: "USR-123", username: "Test", for: "conversation", with: nil).subscribe(onError: { error in
                networkError = error
            })
        ...
        it("invites a user to a conversation with media support") {
            self.stub(file: .inviteUser, request: MembershipRouter.invite(username: "test", conversationId: "conversation").urlRequest)
