...
internal class MemberModelTest: QuickSpec {
    ...
    override func spec() {
        ...
        it("creating member object with json") {
            ...
            let member = try? JSONDecoder().decode(MemberModel.self, from: json)
        ...
        it("creates a model with member json") {
            let json = self.json(path: .member)
            let member = try? MemberModel(json: json, state: .joined)
        ...
        it("fails to create member model") {
            expect(try? JSONDecoder().decode(MemberModel.self, from: [:])).to(beNil())
            expect(try? JSONDecoder().decode(MemberModel.self, from: ["member_id": ""])).to(beNil())
            expect(try? JSONDecoder().decode(MemberModel.self, from: ["member_id": "", "name": ""])).to(beNil())
            expect(try? JSONDecoder().decode(MemberModel.self, from: ["member_id": "", "name": "", "state": ""])).to(beNil())
            expect(try? JSONDecoder().decode(MemberModel.self, from: ["member_id": "", "name": "", "state": "invited"])).to(beNil())
        }

        it("fails to create member model with state") {
            expect(try? MemberModel(json: [:], state: .joined)).to(beNil())
            expect(try? MemberModel(json: ["member_id": ""], state: .joined)).to(beNil())
            expect(try? MemberModel(json: ["member_id": "", "name": ""], state: .joined)).to(beNil())
        }
