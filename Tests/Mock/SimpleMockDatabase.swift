...
internal struct SimpleMockDatabase {
    ...
    var emptyConversation: DBConversation {
        return DBConversation(conversation: ConversationModel(uuid: "con-3", name: "conversation 3", sequenceNumber: 3, members: [], created: Date(), displayName: "conversation 3", state: .invited, memberId: "mem-3"))
    }
    ...
    private func setup() {
        do {
            try ConversationClient.instance.storage.databaseManager.user.insert(user1)
            try ConversationClient.instance.storage.databaseManager.user.insert(user2)
            try ConversationClient.instance.storage.databaseManager.user.insert(user3)
            try ConversationClient.instance.storage.databaseManager.member.insert(DBMember1)
            try ConversationClient.instance.storage.databaseManager.member.insert(DBMember2)
            try ConversationClient.instance.storage.databaseManager.member.insert(DBMember3)
            try ConversationClient.instance.storage.databaseManager.receipt.insert(receipt1)
            try ConversationClient.instance.storage.databaseManager.receipt.insert(receipt2)
            ...
            try ConversationClient.instance.storage.databaseManager.conversation.insert(conversation1.data)
            try ConversationClient.instance.storage.databaseManager.conversation.insert(conversation2.data)

            /// Hack to always get members to refresh
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.3, execute: {
                conversation1.refreshMembers()
                conversation1.refreshEvents()
                conversation2.refreshMembers()
                conversation2.refreshEvents()
            })
        } catch {
            fatalError()
        }
